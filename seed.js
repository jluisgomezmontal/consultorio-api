...
async function ensureConsultorio(data) {
  const existing = await Consultorio.findOne({ name: data.name });
  if (existing) {
    return Consultorio.findByIdAndUpdate(
      existing._id,
      {
...
async function main() {
  ...
  // Ensure doctor records exist
  doctorPrincipal = doctorPrincipal || (await User.findOne({ email: doctorEmail }));
  doctorGuadalupe = doctorGuadalupe || (await User.findOne({ email: doctorGuadalupeEmail }));
  doctorLosArcos = doctorLosArcos || (await User.findOne({ email: doctorLosArcosEmail }));
  doctorMediCare = doctorMediCare || (await User.findOne({ email: doctorMediCareEmail }));
  ...
  const citasData = [
    {
      pacienteId: pacienteCarlos._id,
      doctorId: doctorPrincipal._id,
      consultorioId: consultorio1._id,
      date: addDays(0),
      time: '09:00',
    ...
    {
      pacienteId: pacienteRoberto._id,
      doctorId: doctorPrincipal._id,
      consultorioId: consultorio1._id,
      date: addDays(-2),
      time: '16:00',
    ...
    {
      pacienteId: pacienteLaura._id,
      doctorId: doctorGuadalupe._id,
      consultorioId: consultorio2._id,
      date: addDays(2),
      time: '10:15',
    ...
    {
      pacienteId: pacienteSofia._id,
      doctorId: doctorLosArcos._id,
      consultorioId: consultorio3._id,
      date: addDays(-1),
      time: '12:00',
    ...
    {
      pacienteId: pacienteEduardo._id,
      doctorId: doctorLosArcos._id,
      consultorioId: consultorio3._id,
      date: addDays(4),
      time: '09:30',
    ...
    {
      pacienteId: pacienteLucia._id,
      doctorId: doctorMediCare._id,
      consultorioId: consultorio4._id,
      date: addDays(1),
      time: '17:00',
  ...
  for (const data of citasData) {
    const existing = await Cita.findOne({
    ...
    if (existing) {
      console.log(
        `ℹ️  Appointment for patient at ${data.date.toISOString().split('T')[0]} ${
          data.time
  ...
  for (const pagoData of pagosData) {
    const existing = await Pago.findOne({ citaId: pagoData.citaId });
    if (existing) {
      console.log(`ℹ️  Payment for cita ${pagoData.citaId} already exists, skipping`);
    } else {
      await Pago.create(pagoData);
    }
  }
